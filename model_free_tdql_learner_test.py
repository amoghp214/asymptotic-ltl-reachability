...
from model_free_tdql_learner import TDQL_PAC_Stage
...
def seed_tables(stage, rows, goals=()):
    ...
    for s, acts in rows.items():
        ...
        if s in goals:
            # mirrors handle_new_state, which records goal-ness at visit time so that
            # deflate_ec never has to ask the simulator about a remembered state
            stage.goal_seen.add(s)
        for a in acts:
...
class StrictInterfaceViolation(AssertionError):
    pass


class StrictSimulator:
    """
    A simulator that permits only the three operations the learner is allowed:
    reset to s0, ask for the actions available at the state it is standing in, and
    sample an action at that state. Any query about another state raises.

    Wrapping the real simulator and running the learner under it is what turns
    "the learner is model-free" from a claim into a check.
    """

    def __init__(self, inner):
        self._inner = inner
        self._at = inner.gt_mdp.initial_state
        self.gt_mdp = _StrictMDP(inner.gt_mdp, self)
        self.violations = []

    def _require_here(self, s, what):
        # Resetting to s0 is one of the three permitted operations, and the learner
        # performs it implicitly by starting an episode there rather than announcing
        # it. A query about s0 is therefore treated as that reset.
        if s == self._inner.gt_mdp.initial_state:
            self._at = s
            return
        if s != self._at:
            self.violations.append((what, s, self._at))
            raise StrictInterfaceViolation(
                "%s asked about %r while standing at %r" % (what, s, self._at))

    def reset(self):
        self._at = self._inner.gt_mdp.initial_state
        return self._at

    def step(self, s, a):
        # an episode restart is a reset to s0; anything else must be where we stand
        self._require_here(s, "step")
        s2, r = self._inner.step(s, a)
        self._at = s2
        return s2, r


class _StrictMDP:
    """Exposes initial_state, and per-state queries only for the current state."""
    ...
    def goal_states(self):
        return _GoalProbe(self._inner.goal_states, self._guard)

    def get_actions(self, s):
        self._guard._require_here(s, "get_actions")
        return self._inner.get_actions(s)

    def sample_random_action_from_state(self, s):
        self._guard._require_here(s, "sample_random_action_from_state")
        return self._inner.sample_random_action_from_state(s)

    def is_goal_state(self, s):
        self._guard._require_here(s, "is_goal_state")
        return self._inner.is_goal_state(s)

    def __getattr__(self, name):
        raise StrictInterfaceViolation("model attribute %r is not part of the interface" % name)


class _GoalProbe:
    """`s in goal_states` is allowed only for the state the walk is standing in,
    or the one it has just stepped into."""

    def __init__(self, goals, guard):
        self._goals = goals
        self._guard = guard
    ...
    def __iter__(self):
        raise StrictInterfaceViolation("enumerating the goal set is not part of the interface")

    def __len__(self):
        raise StrictInterfaceViolation("the size of the goal set is not part of the interface")


class TestModelFreeInterface(unittest.TestCase):
    """The learner may only reset, ask for actions here, and sample here."""

    def _drive(self, windowed):
        random.seed(31)
        strict = StrictSimulator(six_state_simulator())
        prev = None
        for k in range(1, 4):
            stage = TDQL_PAC_Stage(k=k, error=1 / 2 ** k, confidence_error=1 / 2 ** k,
                                   p_min=1 / 2 ** k, mu=0.01, mdp_simulator=strict,
                                   prev_stage=prev)
            stage.reset_evidence_each_round = windowed
            stage.sweep_each_round = windowed
            stage.skip_saturated = windowed
            stage.run()
            prev = stage
        return strict, stage

    def test_runs_without_touching_anything_outside_the_interface(self):
        strict, _ = self._drive(windowed=False)
        self.assertEqual(strict.violations, [])

    def test_windowed_mode_also_stays_inside_the_interface(self):
        strict, _ = self._drive(windowed=True)
        self.assertEqual(strict.violations, [])

    def test_the_guard_actually_catches_an_out_of_state_query(self):
        """Without this the two tests above could be passing vacuously."""
        strict = StrictSimulator(six_state_simulator())
        strict.reset()
        with self.assertRaises(StrictInterfaceViolation):
            strict.gt_mdp.get_actions(4)          # not where we are standing, and not s0
        with self.assertRaises(StrictInterfaceViolation):
            strict.gt_mdp.transition_probabilities  # not part of the interface

    def test_bounds_are_still_sound_under_the_strict_simulator(self):
        _, stage = self._drive(windowed=False)
        for s in stage.u:
            for a in stage.u[s]:
                self.assertLessEqual(stage.l[s][a], stage.u[s][a])
